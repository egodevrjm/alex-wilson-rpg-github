...
const handler = createMcpHandler(
  (server: any) => {
    ...
    server.tool('export_room_context', 'Export room rules and eligible people for prompting.', { roomId: z.string() }, async ({ roomId }: any) => {
      const room = findRoom(roomId);
      if (!room) return { ...jsonText({ error: `Unknown room id: ${roomId}` }), isError: true };
      return jsonText({ room, eligible_people: people.filter(person => person.eligible_rooms?.includes(roomId)) });
    });
